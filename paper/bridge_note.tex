...
\item \textbf{Shared inputs:} the same five carrier
      ...
      $\Lambda_{\mu\nu}$ of Paper~4~\cite{Paper4,Paper4B}, which
      at the trace level reads
      $\mathrm{tr}\,\Lambda = \axi^{2}-\gamma^{2}/2 = 161/200
      = 0.805$; at per-axis resolution Paper~4B reports a stable
      $2{+}1$ sign pattern
      $\Lambda_{\mu\nu}=\mathrm{diag}(\axi^{2},
      -\gamma^{2}/2,-\gamma^{2}/2,+\gamma^{2}/2)$ across all
      $N\!\in\![28,200]$, as a derived shared output
      identifying the time--time and spatial-isotropic components
      of $\Lambda_{\mu\nu}$ in the emergent Einstein equation as
      rational structural functions of $\axi$ and $\gamma$ rather
      than free parameters; the multi-observable indirect-witness
      cross-validation of this identification is reported by the
      H3C cluster~\cite{Paper4C}.
      Figure~\ref{fig:lambda_2plus1_panel} shows the diagonal
      $\Lambda_{\mu\mu}$ entries with the trace value
...
$2{+}1$ anisotropic form: time--time component $\alpha_{\xi}^{2}\!=\!81/100$,
two negative spatial axes $-\gamma^{2}/2\!=\!-1/200$, one positive
spatial axis $+\gamma^{2}/2\!=\!+1/200$; trace
$=\alpha_{\xi}^{2}-\gamma^{2}/2=161/200=0.805$.}
...
\footnote{\url{https://github.com/[anonymized]/qft-einstein-bridge-repro}}
