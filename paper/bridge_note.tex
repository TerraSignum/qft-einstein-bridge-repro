...
\item \textbf{Shared inputs:} the same five carrier
      ...
      $\theta_{\rm chir}\!=\!\pi/4$~\cite{Paper4}: matter-branch
      $\mathrm{tr}\,\Lambda \!=\! \axi^{2}\!=\!81/100$ and
      vacuum-branch $\mathrm{tr}\,\Lambda \!=\! \axi^{2}\!+\!\gamma^{2}
      \!=\!41/50$ (both lying inside the canonical $5$-point PRE-flip
      and $7$-point mixed-ladder bootstrap CIs, while the legacy
      sign-flipped trace $\axi^{2}\!-\!\gamma^{2}/2\!=\!161/200$
      is rejected on the $5$-point PRE-flip ladder).
      ...
      $\axi$ and $\gamma$ rather than free parameters;
      the multi-observable indirect-witness cross-validation of
      this identification is reported by the
      H3C cluster~\cite{Paper4C}.
      Figure~\ref{fig:lambda_2plus1_panel} shows the diagonal
      $\Lambda_{\mu\mu}$ sign pattern and the branch-resolved
      trace values.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig_lambda_2plus1_panel.pdf}
\caption{Cosmological-tensor diagonal $\Lambda_{\mu\nu}$:
the per-node $T$-eigenframe carries a stable $2{+}1$
\emph{sign} pattern $(-,-,+)$ across the canonical
$\mathcal P_{5}/\mathcal P_{5}N$ ladder $N\!\in\![50,512]$
(Paper~4B). The legacy magnitude assignment with two negative spatial axes
$-\gamma^{2}/2\!=\!-1/200$ and one positive spatial axis
$+\gamma^{2}/2\!=\!+1/200$ (giving trace
$\alpha_{\xi}^{2}\!-\!\gamma^{2}/2\!=\!161/200\!=\!0.805$)
is superseded by the branch-resolved trace of
Paper~4~\cite{Paper4}: matter-branch
$\mathrm{tr}\,\Lambda\!=\!\alpha_{\xi}^{2}\!=\!81/100$ and
vacuum-branch $\mathrm{tr}\,\Lambda\!=\!\alpha_{\xi}^{2}\!+\!
\gamma^{2}\!=\!41/50$.}
\label{fig:lambda_2plus1_panel}
\end{figure}
...
\item \emph{Cosmological-constant problem (122-orders-of-magnitude
      ...
      \path|src/extend_cc_dressing_to_extra_regimes.py|).
...
\path|src/make_version_lock_table_tex.py| at the moment this
...
\input{tables/tab_version_lock_matrix.tex}
